...
func PrintProbeResult(c *k8s.Client, o Options) error {
	...
	if isKubeArmorRunning(c, o) {
		...
		mp, err := probeRunningKubeArmorNodes(c, o)
		if err != nil {
			log.Println("error occured when probing kubearmor nodes", err)
		}
		err = getAnnotatedPods(c, mp)
		if err != nil {
...
func probeRunningKubeArmorNodes(c *k8s.Client, o Options) (map[string]string, error) {
	...
	if len(nodes.Items) > 0 {
		...
		for i, item := range nodes.Items {
			...
			err = readDataFromKubeArmor(c, o, item.Name, mp)
			if err != nil {
	...
	fmt.Println(mp["filePosture"] + " " + mp["networkPosture"])
	return mp, nil
}

func readDataFromKubeArmor(c *k8s.Client, o Options, nodeName string, mp map[string]string) error {
	...
	var setPostureData = func() {
		mp["filePosture"] = kd.ContainerDefaultPosture.FileAction
		mp["capabilitiesPosture"] = kd.ContainerDefaultPosture.CapabilitiesAction
		mp["networkPosture"] = kd.ContainerDefaultPosture.NetworkAction
	}
	setPostureData()
...
func getSecurityPostureAndVisibility(c *k8s.Client, defposture map[string]string) (map[string]*PostureAndVisibility, error) {
	...
	fmt.Println(defposture["filePosture"] + " " + defposture["capabilitiesPosture"])
	for _, ns := range namespaces.Items {

		filePosture := defposture["filePosture"]
		capabilityPosture := defposture["capabilitiesPosture"]
		networkPosture := defposture["networkPosture"]
		// x, _ := c.K8sClientset.CoreV1().Pods(ns.Name).Get(context.Background(), p.Name, metav1.GetOptions{})

		// fmt.Println(x.Spec.NodeName)
		...
		mp[ns.Name] = &PostureAndVisibility{posture: "file(" + filePosture + "), capabilities(" + capabilityPosture + "), network(" + networkPosture + ")", visibility: ns.Annotations["kubearmor-visibility"]}
...
func getAnnotatedPods(c *k8s.Client, defposture map[string]string) error {
	...
	mp, err := getSecurityPostureAndVisibility(c, defposture)
	if err != nil {
		return err
	}
	...
	for _, p := range pods.Items {
		...
		if p.Annotations["kubearmor-policy"] == "enabled" {
			...
			data = append(data, []string{armoredPod.Namespace, mp[armoredPod.Namespace].posture, mp[armoredPod.Namespace].visibility, armoredPod.Name, ""})
			...
			for policyKey, policyValue := range policyMap {
				...
				if s2.IsSubset(labels) {
					if checkIfDataAlreadyContainsPodName(data, armoredPod.Name, policyKey) {
						...
						data = append(data, []string{armoredPod.Namespace, mp[armoredPod.Namespace].posture, mp[armoredPod.Namespace].visibility, armoredPod.Name, policyKey})
	...
	table.SetHeader([]string{"NAMESPACE", "SECURITY POSTURE", "VISIBILITY", "NAME", "POLICY"})
